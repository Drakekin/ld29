...
static int _cffi_const_H3DRootNode(PyObject *lib)
{
  PyObject *o;
  int res;
  if (LONG_MIN <= (H3DRootNode) && (H3DRootNode) <= LONG_MAX)
    o = PyInt_FromLong((long)(H3DRootNode));
  else if ((H3DRootNode) <= 0)
    o = PyLong_FromLongLong((long long)(H3DRootNode));
  else
    o = PyLong_FromUnsignedLongLong((unsigned long long)(H3DRootNode));
  if (o == NULL)
    return -1;
  res = PyObject_SetAttrString(lib, "H3DRootNode", o);
  Py_DECREF(o);
  if (res < 0)
    return -1;
  return 0;
...
static int _cffi_e_enum_NodeParams(PyObject *lib)
{
  ...
  return _cffi_const_H3DRootNode(lib);
}
...
PyMODINIT_FUNC
init_cffi__x1ae554b7x2b5cba08(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__x1ae554b7x2b5cba08", _cffi_methods);
  if (lib == NULL || 0 < 0)
    return;
  _cffi_init();
  return;
